...
void Floor_state(Player* player, Store* store,int x) {
	int i = 0, j = 0;
	Floor* dest_floor = player->building->floor + x;
...
void rent(Player* player, Store* store,int x) {
	...
	while (1) {
		...
		if (menu == 1) {
			for (i = 0; i < size; i++) {
				for (j = 0; j < size; j++) {
					if (dest_floor->cell[i + j].rentrequest == 1) {
						dest_floor->cell[i + j].rentrequest = 0;
					}
				}
			}
			continue;
			...
			for (i = 0; i < size; i++) {
				for (j = 0; j < size; j++) {
					if (dest_floor->cell[i + j].rentrequest == 1) {
						dest_floor->cell[i + j].id = store->id;
						dest_floor->cell[i + j].rentrequest = 0;
						break;
					}
				}
...
void kick(Player* player, Store* store, int x,int idnum) {
	...
	for (i = 0; i < size; i++) {
		for (j = 0; j < size; j++) {
			if (dest_floor->cell[i + j].id==idnum) {
				dest_floor->cell[i + j].id=0;
				printf("○");
			}
		}
	}
